...
\end{verbatim}
  ...
  \def\labelenumii{\alph{enumii}.}
  \item
    How many total hypothesis tests are being run in this ANOVA table?
  \item
    Use a decision rule that rejects an individual null hypothesis when
    the p-value is less than or equal to \(\alpha = .05\), what is the
    probability of making at least one Type I error across all of these
    tests? (Assume the tests are independent for this calculation).
  \item
...
\item
  ...
  \def\labelenumii{\alph{enumii}.}
  \item
    Why would it be difficult to use \emph{parent} as a blocking factor
    in this study?
  \item
