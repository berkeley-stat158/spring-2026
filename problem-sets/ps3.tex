...
\item
  ...
  \item
    ...
    \tightlist
    \item
      i. for each component of the model (main effect of poisons, main
      effect of drugs, interaction effects), calculate the value of
      observed F statistics and the corresponding p-values. You may
      calculate the p-values using either randomization-based inference
      or model-based inference (or both).
    \item
      ii. draw conclusions about differences among poisons, differences
      among treatments, and interactions between them. Is this a setting
      where it is important to estimate the interaction effects?
    \end{itemize}
  \end{enumerate}
\end{enumerate}
